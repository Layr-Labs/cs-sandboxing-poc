...
IMAGE_REF="docker.io/saucelord/cs-sandboxer:latest"
...
create() {
    ...
    gcloud compute instances create $INSTANCE_NAME \
        ...
        --metadata="^~^tee-image-reference=$IMAGE_REF~tee-container-log-redirect=true~tee-env-IMAGE=$WORKLOAD_IMAGE~tee-cgroup-ns=true~tee-added-capabilities=[\"CAP_SYS_ADMIN\",\"CAP_SYS_RESOURCE\",\"CAP_NET_ADMIN\"]" \
...
setup_firewall() {
    echo "Creating firewall rule to allow health check on port 8080..."
    gcloud compute firewall-rules create allow-health-check-8080 \
        ...
        --description="Allow health check access on port 8080"

    echo ""
    echo "Firewall rule created."
    echo "Instances with tag 'allow-health-check' can now receive traffic on port 8080."
}

delete_firewall() {
    echo "Deleting firewall rule..."
    gcloud compute firewall-rules delete allow-health-check-8080 \
        --project=$PROJECT \
        --quiet
...
get_ip() {
    ...
    echo ""
    echo "Health check endpoint: http://$EXTERNAL_IP:8080/health"
}
...
case "$1" in
    ...
    ip)
        ...
        ;;
    *)
        echo "Usage: $0 {build|delete|create|logs|setup-firewall|delete-firewall|ip}"
        echo ""
        echo "Examples:"
        ...
        echo "  $0 setup-firewall                              # Create firewall rule for health check"
        echo "  $0 ip                                          # Get instance IP and health endpoints"
        echo "  $0 ip my-instance                              # Get IP for specific instance"
        echo "  $0 logs                                        # View logs"
        echo "  $0 logs my-instance                            # View logs for specific instance"
        echo "  $0 delete                                      # Delete instance"
        echo "  $0 delete-firewall                             # Delete firewall rule"
        exit 1
        ;;
esac
